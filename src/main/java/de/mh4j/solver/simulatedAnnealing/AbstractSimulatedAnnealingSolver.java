    ...
    protected void doStep() {
        ...
        if (neighborSolution.isBetterThan(currentSolution)) {
            ...
            if (randomizer.nextDouble() < acceptanceProbability) {
                currentSolution = neighborSolution;
                log.debug("Took worse neighbor because of the cooling scheme acceptance probability. New costs are {}",
                        neighborSolution.getCosts());
            }
            else {
                log.debug("Neighbor is worse than the current configuration. Costs stay at {}",
                        currentSolution.getCosts());
            }
            situationHasNotImproved++;
        }
